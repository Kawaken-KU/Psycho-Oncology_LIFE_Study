...
ggplot(monthly_data_depression, aes(x = month_since_entry)) +
  ...
  scale_x_continuous(
    breaks = seq(0, max(monthly_data_depression$month_since_entry), by = 2),
    labels = seq(0, max(monthly_data_depression$month_since_entry), by = 2)
  ) +
  theme_minimal()+
  theme(legend.position = "bottom")
```
...
# グループ化
group_data_depression <- df_solidcancer |> 
  group_by(sex, age_group, Ope, Chemotherapy, Radiation) |> 
  summarise(
    Event = sum(Event_depression),
    Persontime = sum(risk_months_depression)
  ) |> 
  ungroup()
